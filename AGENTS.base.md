...
- 文本文件统一使用 `LF`（`\n`）换行。  
- 禁止提交包含 `CRLF`（`\r\n`）的源码、文档、配置文件（Windows 脚本可按项目约定例外）。  
- 修改现有文件时，保持并统一为 `LF`，避免在一次提交中引入无意义的整文件换行变更。  
- 建议通过 `.gitattributes` 显式约束：`*.go text eol=lf`、`*.proto text eol=lf`、`*.md text eol=lf`。  
...
- **将一段线性流程拆成多个“单行/一两行”的小方法，且无复用价值，导致阅读路径被迫频繁跳转。**  
  推荐做法：优先在一个方法内用清晰的中文“步骤注释”把流程写完整；只有当存在复用、或显著降低复杂度/重复时才抽取方法。
...
- **Windows + VSCode 场景下，禁止运行可能在源码目录生成并残留临时文件的“原地覆写”格式化/写入命令（典型如 `gofmt -w` 产生 `*.go.<数字>`）。**  
  如需格式化：优先使用“`gofmt` 输出内容 → 通过补丁方式写回文件”的方式，避免在源码目录落地临时文件。  
  如仍意外生成临时文件：必须在本次任务内清理；若因文件占用导致无法清理，需明确列出残留文件路径，并给出可执行的释放占用/清理步骤。  
