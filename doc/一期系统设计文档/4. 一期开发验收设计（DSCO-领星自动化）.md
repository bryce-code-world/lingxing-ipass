...
4. 启动业务进程：`go run ./cmd/ipass/main.go`（提供闭环调度 + ops HTTP）
5. 准备 `.env.admin`：从 `.env.admin.example` 复制并填好 admin DB/密码/ops 地址等
6. 启动 admin 进程：`go run ./cmd/admin/main.go`（提供 Admin UI）
7. 打开 Admin UI：`/admin/ui/login` 登录 → `/admin/ui/` 总览确认页面可用
...
重要注意：
- 当前程序采用“两进程分离”：
  - 业务进程（`cmd/ipass`）提供 ops HTTP（`/admin/*`）用于触发任务
  - admin 进程（`cmd/admin`）提供可视化后台（UI + API），并通过 HTTP 调用业务 ops 触发任务
- 如果你要通过 ops 触发某个 job，请确保业务侧已启用对应能力与配置（例如 DSCO/领星 token、sid、库存映射等），否则会在运行时返回错误。
...
- `IPASS_OPS_PASSWORD`：业务进程 ops 接口鉴权密码（调用业务 `/admin/*` 必须带 `X-Ops-Password`）
...
admin 进程与业务进程完全分离，分别有各自的鉴权口径：

- admin UI/API（admin 进程）：
  - 登录密码：`ADMIN_PASSWORD`
  - 浏览器：`GET /admin/ui/login` 登录（session cookie）
  - 脚本：请求头 `X-Admin-Password: <ADMIN_PASSWORD>`
- 业务 ops（业务进程）：
  - 鉴权密码：`IPASS_OPS_PASSWORD`
  - 调用业务 `/admin/*` 必须带 `X-Ops-Password: <IPASS_OPS_PASSWORD>`
...
- 启动业务进程：`go run ./cmd/ipass/main.go`
- 启动 admin 进程：`go run ./cmd/admin/main.go`
...
> 注意：
> - 触发任务属于“业务 ops”：实际调用的是业务进程的 `/admin/run`，请求头必须携带 `X-Ops-Password: <IPASS_OPS_PASSWORD>`。
> - 你也可以通过 admin UI 的“任务”页面触发（admin 会代你调用业务 ops）。
...
```bash
curl -H "X-Ops-Password: change-me-ops" http://127.0.0.1:8080/healthz
curl -H "X-Ops-Password: change-me-ops" "http://127.0.0.1:8080/admin/order_state/get?dsco_order_id=d1"
curl -H "X-Ops-Password: change-me-ops" -X POST "http://127.0.0.1:8080/admin/run?job=pull_dsco_orders"
```

示例（PowerShell）：

```powershell
Invoke-RestMethod -Headers @{ "X-Ops-Password"="change-me-ops" } -Method Get  -Uri "http://127.0.0.1:8080/healthz"
Invoke-RestMethod -Headers @{ "X-Ops-Password"="change-me-ops" } -Method Post -Uri "http://127.0.0.1:8080/admin/run?job=pull_dsco_orders"
```

### 12.2 水位设置（验收常用）

```bash
curl -H "X-Ops-Password: change-me-ops" -X POST \
