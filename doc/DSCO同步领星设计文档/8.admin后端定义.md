...
- 订单一致性检查（不落库）：`POST /admin/api/dsco_order_sync/check`
...
- `dsco_status`
- `status`（右侧可显示检查结果临时标记：✓/!；仅当前页面有效）
- `warehouse-shipment`：`{warehouse_id}-{shipment}`（节省横向空间）
- `dsco_retailer_id`
- `mskus`（展示为多行；元素格式为 `sku(quantity)`，保留重复行）
- `shipped_tracking_no`
- `dsco_invoice_id`
- actions：`View`（查看详情）、`Check`（对单个订单执行检查）、`Run`（仅对单个订单执行任务）、`Edit`（编辑 status）

页面交互补充（与实现一致）：
- 列表支持列显隐开关（Columns），默认隐藏：`id`、`dsco_retailer_id`；用于减少横向滚动。
- 范围检查入口：页面按钮 `Check Range`，使用当前输入的 `start/end` 时间范围；执行后在列表 `status` 旁临时标记 ✓/!，并在存在 `missing_local_record` 时弹窗展示缺失列表。
...
API：
- `POST /admin/api/dsco_order_sync/check`

请求 body（二选一）：
1) 按时间范围（UTC 秒级，边界 `[start,end)`）：
```json
{ "start": 1700000000, "end": 1700086400 }
```
2) 按单个订单：
```json
{ "po_number": "JQ8HLDK5UG781TL2JAV5" }
```

响应（核心结构；字段为 snake_case）：
```json
{
  "results": [
    {
      ...
      "message": "dsco invoice not found by poNumber",
      "local_status": 5,
      "dsco_status": "completed",
      "detail": {}
    }
  ],
  "missing_local": [
    { "po_number": "PO-XXX", "dsco_status": "created", "dsco_order_id": "1078..." }
  ],
  "meta": {
    "dsco_order_count": 120,
    "local_order_count": 98
  }
}
```

说明：
- `status=1` 的订单不检查（不返回对应结果）；检查结果不落库，仅在前端内存中保留到页面刷新为止。
- `missing_local` 用于提示“DSCO 有单但本地没有入库记录”的异常（可能漏拉单/漏入库）。
