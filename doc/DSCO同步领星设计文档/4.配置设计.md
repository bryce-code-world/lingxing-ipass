...
- 各任务开关的“启用依赖”：
  ...
  - 启用任何领星相关任务前：`integration.lingxing.base_url`、`integration.lingxing.access_token`、`integration.lingxing.platform_code`、`integration.lingxing.store_id`（env.yaml）以及 `auth.lingxing.app_id/auth.lingxing.app_secret`（env.yaml）必须存在。
