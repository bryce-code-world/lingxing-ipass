...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `integration.dsco.base_url` | string | 是 | DSCO API Base URL |
| `integration.dsco.codes` | object | 否 | DSCO 侧业务数据编码配置（结构按实现迭代；仅做透传存放） |
| `integration.lingxing.base_url` | string | 是 | 领星 API Base URL |
| `integration.lingxing.access_token` | string | 是 | 领星 API access_token（运行时调用使用；如未来改为自动换 token，可调整实现） |
| `integration.lingxing.platform_code` | int | 是 | 领星平台编码（与业务需求一致） |
| `integration.lingxing.store_id` | string | 是 | 领星店铺/渠道标识 |
| `integration.lingxing.sid` | int | 是 | 领星 `sid`（用于发货回传等） |
| `integration.shipment.ship_date_source` | string | 是 | 发货回传 `shipDate` 口径：`delivered_at` / `stock_delivered_at` / `none` |
...
integration:
  ...
  lingxing:
    ...
    access_token: "lingxing_access_token_here"
    platform_code: 0
    store_id: ""
    sid: 0
  shipment:
    ship_date_source: "delivered_at" # delivered_at/stock_delivered_at/none
...
| 字段 | 类型 | 必填 | 映射方向（约定） | 说明 |
| --- | --- | --- | --- | --- |
| `mapping.shop` | object(map) | 否 | DSCO 店铺/渠道 ID → 领星 store_id | 多店铺/多渠道时使用；单店铺可为空（使用 `integration.lingxing.store_id`） |
| `mapping.warehouse` | object(map) | 是 | DSCO warehouseCode → 领星仓库 ID（WID） | 库存同步必需；key/value 都是字符串（例如 value 为 `"26"`） |
| `mapping.sku` | object(map) | 否 | DSCO partnerSku → 领星 MSKU（或 SKU） | 推单时使用；缺省可按“同名直传”处理 |
| `mapping.shipment` | object(map) | 否 | DSCO shipMethod → 领星 logistics_type_name | 回传物流时需要反向映射（领星 → DSCO） |
...
- 各任务开关的“启用依赖”：
  ...
  - 启用任何领星相关任务前：`integration.lingxing.base_url`、`integration.lingxing.platform_code`、`integration.lingxing.store_id`（env.yaml）以及 `auth.lingxing.app_id/auth.lingxing.app_secret`（env.yaml）必须存在；`integration.lingxing.access_token` 可选（为空时由 SDK AutoToken 自动获取/续约）。
  - 启用发货回传任务前：`integration.lingxing.sid` 必须为正整数，且 `integration.shipment.ship_date_source` 必须为合法枚举值。
  - 启用库存同步任务前：`mapping.warehouse` 必须非空。
> 说明：敏感字段（例如 `integration.lingxing.access_token`）不进入 Admin 配置页面，仅通过部署配置管理。
