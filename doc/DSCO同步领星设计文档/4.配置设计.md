...
- 领星 `store_id`：由 DSCO 订单 `dscoRetailerId` 通过 `runtime_config.mapping.shop` 映射得到。
- 领星 WMS 查询 `sid_arr`：一期取 `runtime_config.mapping.shop` 的映射值并尝试解析为 int（用于 `WmsOrderList.sid_arr`）。
...
```json
{
  ...
  "jobs": {
    ...
    "pull_dsco_orders": { "enable": false, "cron": "0 5 * * * *", "size": 50 },
    "push_to_lingxing": { "enable": false, "cron": "0 10 * * * *", "size": 50 },
    "ack_to_dsco": { "enable": false, "cron": "0 15 * * * *", "size": 50, "multi_ban": false },
    "ship_to_dsco": { "enable": false, "cron": "0 20 * * * *", "size": 50, "multi_ban": false },
    "invoice_to_dsco": { "enable": false, "cron": "0 25 * * * *", "size": 50, "multi_ban": false },
    "sync_stock": { "enable": false, "cron": "0 30 1 * * *", "size": 50, "sync": false, "use_stream": false },
    "pull_sku_pair": { "enable": false, "cron": "0 0 3 * * *", "size": 1000 }
  },
  "mapping": {
    "shop": { "1000011436": "110658143132021760" },
    "warehouse": { "YQN-CA": "33328", "YQN-CA2": "28393" },
    "sku": {},
    "shipment": {
...
字段说明（关键口径）：
- `jobs.*.cron`：cron 表达式（UTC 时区）；使用 6 段（含秒）格式。
- `jobs.*.size`：单次执行处理的最大条数上限（所有任务统一都有该字段）。
- `jobs.pull_sku_pair`：每日自动拉取领星配对关系并更新 `mapping.sku`；`size` 用作分页大小（建议 1000+）；导出审计文件落盘到 `exports/sku-mapping-pull/{yyyy-MM-dd}/`，由 `cleanup_exports` 统一清理。
- `mapping.shop`：店铺/渠道 ID 映射（key/value 都按字符串处理）。
- `mapping.warehouse`：仓库 ID 映射（key/value 都按字符串处理）。
- `mapping.sku`：SKU 映射（配置方向为 DSCO SKU -> 领星 SKU；推单到领星时可用于补齐 sku；回传时需构建反向映射：领星 SKU -> DSCO partnerSku）。
- `mapping.shipment`：物流方式映射（用于领星建单的 `LogisticsTypeID`；key 为 `<ShipWarehouseCode(空则用 RequestedWarehouseCode)>-<ShippingServiceLevelCode>`）。
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `jobs.<job>.enable` | bool | 是 | 是否纳入定时调度；`false` 表示不定时触发（但 Admin 手动触发不受此开关限制） |
| `jobs.<job>.cron` | string | 是 | cron 表达式（UTC 时区） |
| `jobs.<job>.size` | int | 是 | 单次执行处理条数上限（分页大小/批量大小的统一口径） |
| `jobs.<job>.sync` | bool | 否 | 是否执行“写回/同步”动作；目前仅 `sync_stock` 任务读取该配置；默认 `false`（仅拉取并记录，不回写 DSCO inventory） |
| `jobs.<job>.use_stream` | bool | 否 | 是否优先使用 DSCO Stream + sync operation 拉取 inventory（用于对账读取 DSCO 当前库存）；目前仅 `sync_stock` 任务读取该配置；默认 `false`（按 SKU 调用 `GET /inventory` 并缓存） |
| `jobs.<job>.multi_ban` | bool | 否 | 是否禁止处理“多行/多数量”订单；仅 `ack_to_dsco`/`ship_to_dsco`/`invoice_to_dsco` 任务读取该配置；默认 `false`（不禁止） |
...
| --- | --- | --- | --- |
| `pull_dsco_orders` | 拉取 DSCO 订单入库（按 `created_at` + `[start,end)`） | 单次拉取 DSCO 订单最大条数（分页/游标由实现决定） | 50 |
| `push_to_lingxing` | 将入库订单写入领星 | 单次写入领星的最大订单数 | 50 |
| `ack_to_dsco` | 领星状态满足条件后回传 ACK | 单次回传 ACK 的最大订单数 | 50 |
| `ship_to_dsco` | 回传物流信息 | 单次回传物流的最大订单数 | 50 |
| `invoice_to_dsco` | 回传发票信息 | 单次回传发票的最大订单数 | 50 |
| `sync_stock` | 库存同步 | 单次同步的最大 SKU 数（或明细条数；以实现落地为准） | 50 |
...
| 字段 | 类型 | 必填 | 映射方向（约定） | 说明 |
| --- | --- | --- | --- | --- |
| `mapping.shop` | object(map) | 是 | DSCO dscoRetailerId → 领星 StoreID（字符串） | 推单到领星必需（用于确定 StoreID）；同时该值用于 WmsOrderList 的 `sid_arr`（需能解析为 int） |
| `mapping.warehouse` | object(map) | 是 | DSCO warehouseCode → 领星仓库 ID（WID） | 库存同步必需；key/value 都是字符串（例如 value 为 `"26"`） |
| `mapping.sku` | object(map) | 否 | DSCO SKU（优先 partnerSku，否则 sku） → 领星 SKU | 推单到领星：用于补齐 sku（无映射则不传）；回传/库存回写：构建反向映射（领星 SKU → DSCO partnerSku） |
...
```json
{
  ...
  "jobs": {
    ...
    "ack_to_dsco": { "enable": true, "cron": "0 */15 * * * *", "size": 200, "multi_ban": false },
    "ship_to_dsco": { "enable": true, "cron": "0 */15 * * * *", "size": 200, "multi_ban": false },
    "invoice_to_dsco": { "enable": true, "cron": "0 */15 * * * *", "size": 50, "multi_ban": false },
    "check_orders": { "enable": false, "cron": "0 0 2 * * *", "size": 200 },
    "sync_stock": { "enable": true, "cron": "0 0 0 * * *", "size": 200, "sync": true, "use_stream": true },
    "cleanup_exports": { "enable": true, "cron": "0 0 */1 * * *", "size": 1 }
  },
  "mapping": {
    "shop": {},
...
- `check_orders`：仅用于 Admin 手动检查（不需要定时触发），建议保持 `enable=false`；`cron` 字段仅用于通过配置校验（占位即可）。
...
- 各任务开关的“启用依赖”：
  - 启用任何 DSCO 相关任务前：`auth.dsco.token`（env.yaml）必须存在；`integration.dsco.base_url` 可为空（使用 SDK 默认 BaseURL）。
  - 启用任何领星相关任务前：`integration.lingxing.platform_code`（env.yaml）与 `auth.lingxing.app_id/auth.lingxing.app_secret`（env.yaml）必须存在；`integration.lingxing.base_url` 可为空（使用 SDK 默认 BaseURL）。
  - 启用 `push_to_lingxing` 前：`mapping.shop` 必须覆盖 DSCO 订单的 `dscoRetailerId`；`mapping.shipment` 必须覆盖 `<ShipWarehouseCode(空则用 RequestedWarehouseCode)>-<ShippingServiceLevelCode>`；否则该订单推单会被跳过并记录日志。
  - 启用 `ship_to_dsco`/`invoice_to_dsco` 前：`mapping.shop` 必须非空（用于 WmsOrderList.sid_arr），否则该订单会被跳过并记录日志。
  - 启用库存同步任务前：`mapping.warehouse` 必须非空；`mapping.sku` 可为空（缺省同名直传）。
