...
- `mapping.sku`：SKU 映射（仅用于“库存回传 DSCO”场景：领星 SKU -> DSCO partnerSku；推单到领星不使用该映射）。
...
| `mapping.sku` | object(map) | 否 | DSCO partnerSku → 领星 SKU | 推单到领星不使用；仅用于“库存回传 DSCO”时构建反向映射：领星 SKU → DSCO partnerSku |
