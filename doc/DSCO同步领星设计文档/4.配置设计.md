...
| 字段 | 类型 | 必填 | 映射方向（约定） | 说明 |
| --- | --- | --- | --- | --- |
| `mapping.shop` | object(map) | 是 | DSCO dscoRetailerId（或 channel） → 领星 store_id | 推单到领星必需（用于确定 StoreID） |
...
| `mapping.shipment` | object(map) | 是 | DSCO shippingServiceLevelCode → 领星 SID（字符串） | 发货/发票回传时用于查询领星 WMS 出库单（WmsOrderList.sid_arr） |
