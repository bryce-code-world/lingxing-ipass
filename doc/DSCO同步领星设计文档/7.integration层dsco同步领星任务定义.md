...
- 若本地 `dsco_order_sync.shipped_tracking_no` 已有值，视为已回传过 shipment，直接跳过（不重复回传）。
...
- 本地写回 `shipped_tracking_no` 时，使用**英文逗号**拼接去重后的 tracking 列表（便于幂等与 Admin 筛选）。
...
- 若本地 `dsco_order_sync.dsco_invoice_id` 已有值，视为已回传过发票，直接跳过（不重复回传）。
