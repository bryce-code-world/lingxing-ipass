...
- 因此 `mapping.sku` 一期暂不使用，保留字段仅为未来扩展位（如后续发现领星无法自动匹配，再启用映射）。
...
1) 使用 `lingxing.Warehouse.WmsOrderList` 获取运单号、发货时间与实发数量：
   - 入参：
     ...
     - `sid_arr = [SID]`：
       - SID 获取方式（写死）：从 DSCO 原始订单取 `dscoRetailerId`（或 channel 兜底），通过 `runtime_config.mapping.shop` 映射得到领星 SID（字符串，需可解析为 int）
   - 取值：
     - `tracking_no`（运单号）
