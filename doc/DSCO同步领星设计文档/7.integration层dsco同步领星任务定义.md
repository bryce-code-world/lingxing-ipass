...
用途：用于在领星侧查询 WMS 出库单（WmsOrderList），需要传入 `sid_arr`。

- key：DSCO `shippingServiceLevelCode`（或 requestedShippingServiceLevelCode 兜底）
- value：领星 `SID`（字符串，需可解析为 int）

约束（写死）：

- `ship_to_dsco` / `invoice_to_dsco` 必须能通过 `mapping.shipment` 将 DSCO 订单映射到领星 SID，否则该订单会被跳过并记录日志。
- 不做反向映射（本项目一期不需要 `SID -> shippingServiceLevelCode`）。
...
1) 使用 `lingxing.Warehouse.WmsOrderList` 获取运单号、发货时间与实发数量：
   - 入参：
     ...
     - `sid_arr = [SID]`
       - SID 获取方式（写死）：从 DSCO 原始订单中取 `shippingServiceLevelCode`（或 requestedShippingServiceLevelCode 兜底），再通过 `runtime_config.mapping.shipment` 映射得到领星 SID（字符串，需可解析为 int）。
   - 取值：
     - `tracking_no`（运单号）
