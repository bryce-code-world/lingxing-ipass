...
- 配置方向（写死）：**DSCO SKU（主口径为 `lineItems.sku`，`partnerSku` 仅兼容） -> 领星 SKU**。
- 推单到领星（`push_to_lingxing`）：
  - `msku`：优先使用 DSCO `lineItems.sku`，缺失时兜底 `partnerSku`。
  - `sku`：若 `mapping.sku` 中存在映射，则补齐领星平台维度 `sku`；若无映射则不传（omitempty）。
- 领星 → DSCO 回传（`ship_to_dsco` / `invoice_to_dsco` / `sync_stock`）：
  - 领星侧返回的 `sku` 需要回写为 DSCO SKU 主口径（`lineItems.sku`）；若订单缺失 `sku` 才退化到 `partnerSku`。
...
- 全量门禁（写死）：仅当 `poNumber` 下所有 SKU（主口径 `lineItems.sku`，缺失时兜底 `partnerSku`）满足 `shipped_qty >= expected_qty` 时才允许回传 shipment；未全量发货一律跳过并等待下次任务重试（不调用 DSCO shipment 接口、不推进本地 `status`）。
...
1) 使用 `lingxing.Warehouse.WmsOrderList` 获取运单号、发货时间与实发数量：
   ...
   - 前置条件（写死）：只处理存在 `tracking_no` 的出库单；`tracking_no` 为空则不回传。
2) 若 WmsOrderList 无数据（或筛选后无有效 `tracking_no` 的记录），认为“暂未满足回传条件”，等待下次任务重试（不再用订单详情兜底拼 shipDate）。

3) 全量校验（写死）：
   - `expected_qty`：来自 DSCO 原始订单 `lineItems`（按 SKU 主口径聚合）。
   - `shipped_qty`：来自 `WmsOrderList.product_info[]`（按 `tracking_no` 聚合后，再按 SKU 主口径汇总）。
   - 任一 SKU 出现 `shipped_qty < expected_qty` 时，判定为“未全量发货”，本次不回传 shipment。
...
- 若同一订单行（同一 SKU 主键）跨多个 `tracking_no`，对应 `ShipmentLineItemForUpdate.packageSpanFlag=true`（避免被零售商策略判定为 partial shipment）。
- 本地写回 `shipped_tracking_no` 时，使用**英文逗号**拼接去重后的 tracking 列表（便于展示与 Admin 筛选；不作为是否回传 shipment 的判断依据）。

发货时间口径（已确认，写死）：

- DSCO `shipDate` 统一取自“有 `tracking_no` 的出库单”：优先 `WmsOrder.DeliveredAt`；为空则 `WmsOrder.StockDeliveredAt` 兜底；并转换为 DSCO 要求的 ISO 8601/RFC3339（如 `YYYY-MM-DDTHH:MM:SSZ`）。
...
- 发货行项目：
  ...
  - SKU 映射：领星 SKU 通过 `mapping.sku` 的**反向映射**得到 DSCO SKU（主口径 `lineItems.sku`，缺失时兜底 `partnerSku`；再缺失则同名直传）
  - `packageSpanFlag`：当同一订单行（同一 SKU 主键）分布在多个 `tracking_no` 时，显式传 `true`
  - DSCO 行项目标识：优先写入 `sku`，并在可用时补充 `partnerSku`；如能从 DSCO 原始订单中匹配到 `dscoItemId` 与 `lineNumber`，可额外带上以增强唯一性（提升 DSCO 侧匹配成功率）
...
- 前置条件：`poNumber` 下所有 SKU 均已发货
  - 判断方式：聚合 `WmsOrderList.product_info` 的已发货数量（按 `mapping.sku` 反向映射到 DSCO SKU 主口径），要求对 DSCO 原始订单每个 SKU 满足 `shipped_qty >= expected_qty`。
...
- `lineItems[]`：
  ...
  - 标识字段：优先写 `sku`，并在有值时补 `partnerSku`；同时尽量补齐 `dscoItemId` / `lineNumber`
...
- SKU 映射：领星 `sku` → DSCO SKU（主口径 `lineItems.sku`，缺失时兜底 `partnerSku`），使用 `mapping.sku` 的**反向映射**（缺省同名直传）
