...
| 3 | 已回传 ACK（待回传物流） | `ship_to_dsco` | 4 |
| 4 | 已回传物流（待回传发票） | `invoice_to_dsco` | 5 |
| 5 | 已回传发票（完成） | 无 | 无 |

补充约束（已确认）：

- Admin 手动拉单入库：若订单已存在，允许覆盖 `payload` 与 `status`（可直接置为 5，用于初始化补数据）。
...
- `integration.dsco.base_url`（`env.yaml`）
...
- `integration.lingxing.base_url`、`integration.lingxing.access_token`（`env.yaml`）
...
| `ship_to_dsco` / `invoice_to_dsco` | `lingxing.(*WarehouseService).WmsOrderList` | `POST /erp/sc/routing/wms/order/wmsOrderList` | 查询销售出库单（tracking、delivered_at、stock_delivered_at、实发数量） |
...
- value：领星 `store_id`。
- 若 `mapping.shop` 为空：默认使用 `integration.lingxing.store_id`。
...
- `mapping.sku` **主方向**：`DSCO partnerSku -> 领星 MSKU（或 SKU）`
  - 用于 `push_to_lingxing`（领星创建订单时使用 `msku` 字段）。
- `ship_to_dsco` / `invoice_to_dsco` / `sync_stock` 需要反向映射：`领星 SKU -> DSCO partnerSku`
  - 实现侧在内存中构建反向 map（假设一一映射；若冲突则视为配置错误并拒绝启用相关任务）。
- 若未配置该 SKU：按“同名直传”策略处理（即认为两端 SKU 相同）。

### 5.4 mapping.shipment（物流方式映射）

用途：把 DSCO `shipMethod` 映射到领星 `logistics_type_name`。

- key：DSCO `shipMethod`
- value：领星 `logistics_type_name`

反向映射要求：

- `ship_to_dsco` 回传物流时需要 `logistics_type_name -> shipMethod`，因此实现侧必须构建反向 map。
- 若反向映射为空：`shipMethod` 可不填（但若 DSCO 账号策略要求必须提供，则视为配置缺失并阻止该任务启用）。
...
- 请求：`CreateOrdersV2Request`
  - `platform_code`：固定为 lingxing sdk 中的 `lingxing.PlatformCodeCustom`（10009）常量
...
- 每个订单：`CreateOrderV2`
  ...
  - `items[]`：逐行映射
    - `msku`：由 DSCO `lineItems[].partnerSku` 通过 `mapping.sku` 映射到领星 MSKU（或 SKU）；缺省同名直传
    ...
    - `unit_price`：用于满足领星必填字段，按以下优先级取值：
      ...
      5) 若以上均为空：视为数据缺失，任务应失败并保持状态不推进（避免写入“金额为 0”的脏数据）
...
1) 优先使用 `lingxing.Warehouse.WmsOrderList`（用于“实发数量”与 `stock_delivered_at` 口径）：
   - 入参：`platform_order_no_arr = [poNumber]`，并按需传 `sid_arr`
   - 取值：
     - `tracking_no`（运单号）
     - `delivered_at`（字符串时间）
     - `stock_delivered_at`（字符串时间）
     - `product_info[]`（`sku` + `count`）
2) 若 WmsOrderList 无数据，再使用 `lingxing.Order.GetOrderDetailV2` 兜底：
   - `tracking_no`
   - `logistics_type_name`
   - `global_delivery_time`（秒级时间戳）

发货时间口径（已确认，配置项在 `env.yaml`）：

- `integration.shipment.ship_date_source`：
  - `delivered_at`：使用 WmsOrderList `delivered_at`；若无则用订单详情 `global_delivery_time`
  - `stock_delivered_at`：使用 WmsOrderList `stock_delivered_at`；若无则视为缺失
  - `none`：不回传 `shipDate`
...
- 领星 → DSCO：
  ...
  - `logistics_type_name` → `shipMethod`（通过 `mapping.shipment` 的**反向映射**得到；可选但可能受 DSCO 账号策略约束）
  - `shipDate`：按上面的口径，转换为 DSCO 要求的 ISO 8601/RFC3339（如 `YYYY-MM-DDTHH:MM:SSZ`）
- 发货行项目：
