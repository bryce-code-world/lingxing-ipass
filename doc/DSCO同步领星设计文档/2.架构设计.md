...
│       ├── config_audit_store.go        # 配置审计与回滚
│       ├── job_lock_store.go            # 任务互斥锁：同一任务不并发（定时/手动共享）
│       ├── job_run_store.go             # 任务运行记录（开始/结束/耗时/错误），给 Admin 展示
...
│
├── migrations/
│   ├── 0001_init_pg.sql            # PostgreSQL 初始化建表（禁止运行时 AutoMigrate）
