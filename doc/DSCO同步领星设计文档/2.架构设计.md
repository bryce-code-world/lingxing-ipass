...
- 形态：PostgreSQL 表 `runtime_config` 中按 `domain` 存一份“当前生效配置”（JSON）
- 内容：任务开关/cron/批大小、映射 JSON（shop/warehouse/shipment/sku 等）
...
│   ├── server.go                  # Gin 组装：/admin UI + /admin/api；静态资源与模板初始化
...
- CSV 导出文件落地在部署机器磁盘，目录路径由 `env.yaml` 的 `admin.export_dir` 注入。
...
- 配置位置清晰：启动配置与授权信息在根目录 `env.yaml`；运行时配置在 DB 且可热更新（不做审计表）。
