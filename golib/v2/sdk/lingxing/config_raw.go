...
// GetPairListV2WithRawBody 查询多平台 SKU 配对列表，并返回原始响应 body。
//
// API Path: /pb/mp/listing/v2/getPairList
func (s *ConfigService) GetPairListV2WithRawBody(ctx context.Context, req PairListV2Request) (PairListV2ResponseData, string, error) {
	if req.Offset < 0 {
	...
	_, raw, err := s.c.doSignedJSONWithEnvelopeWithRawBody(ctx, http.MethodPost, "/pb/mp/listing/v2/getPairList", nil, req, &out)
	return out, raw, err
}
